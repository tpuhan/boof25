...
void loop() {
  ...
  if (cycleTime < 3000) {
    ...
    if (step >= 0) fill_solid(leds1, NUM_LEDS, nutBlue);
    if (step >= 1) fill_solid(leds2, NUM_LEDS, nutBlue);
    if (step >= 2) fill_solid(leds3, NUM_LEDS, nutBlue);
    if (step >= 3) fill_solid(leds4, NUM_LEDS, nutBlue);
    if (step >= 4) fill_solid(leds5, NUM_LEDS, nutBlue);
    if (step >= 5) fill_solid(leds6, NUM_LEDS, nutBlue);
  } 
  else {
    // Off phase
